...
| สภาพแสง  | ความต้านทาน   | ค่าที่วัดได้(Ω) |
| -------- | ------------- | --------------- |
| มืดสนิท  | 200 kΩ – 2 MΩ | 1.11            |
| ห้องปกติ | 10 kΩ – 50 kΩ | 1.66            |
| ใต้โคมไฟ | 2 kΩ – 10 kΩ  | 248.9           |
| กลางแจ้ง | 200 Ω – 2 kΩ  | -               |
...
| ----------- | ---- | ---------- | ----------- |
| ใต้โต๊ะ     | 1040 | 30         | 36          |
| ห้องเรียน   | 1666 | 160        | 165         |
| หน้าต่าง    | 4095 | 1165       | 500         |
...
| ----------- | ---- | ---------- | ----------- |
| ใต้โต๊ะ     | 1040 | 10         | 16          |
| ห้องเรียน   | 1666 | 165        | 175         |
| หน้าต่าง    | 4095 | 1163       | 520         |

ให้นักศึกษาวิเคราะห์ว่า

- ความคลาดเคลื่อนอยู่ในระดับที่ยอมรับได้หรือไม่
  ```
  อยู่ในระดับที่ยอมรับได้ เพราะค่าที่วัดจากโค้ดใกล้เคียงกับค่า Lux จากมือถือ มีความคลาดเคลื่อนประมาณ 6% เลยถือว่าไม่มากสำหรับเซนเซอร์ LDR
  ```
  
- สมการ Linear หรือ Polynomial เหมาะกว่ากัน
 ```
Polynomial เหมาะกว่า เพราะความสัมพันธ์ระหว่างค่า ADC กับค่า Lux ไม่เป็นเส้นตรง ทำให้คำนวณค่าได้ใกล้เคียงกับค่าจริงและแม่นยำกว่า Linear
 ```
## **ส่วนที่ 8 — คำถามท้ายใบงาน**

1. ทำไมต้องใช้ Voltage Divider ในการอ่าน LDR?
    ```
    เพราะ LDRเปลี่ยนค่าความต้านทาน ไม่ได้ให้แรงดัน จึงใช้ Voltage Divider เพื่อแปลงเป็นแรงดันให้ ADC อ่านค่าได้
    ```
2. ทำไมต้องเก็บข้อมูลหลายจุดก่อนทำ Calibration?
   ```
   เพื่อให้สมการที่ได้มีความแม่นยำและครอบคลุมช่วงการวัดทั้งหมด
   ```
3. ถ้า Noise มากกว่าสัญญาณจริง จะเกิดอะไรขึ้น?
   ```
   ค่าที่อ่านได้จะไม่เสถียรและวัดค่าได้ไม่ถูกต้อง
   ```
4. ทำไมการวางตำแหน่งเซนเซอร์จึงสำคัญต่อ Signal Integrity?
    ```
    ตำแหน่งมีผลต่อปริมาณแสงที่เซนเซอร์รับได้ ถ้าวางไม่เหมาะสมจะทำให้ค่าที่วัดคลาดเคลื่อน
    ```
5. ถ้า ADC มี Resolution ต่ำ จะส่งผลอย่างไรต่อความแม่นยำ?
    ```
	ค่าที่อ่านได้จะละเอียดน้อยลง ทำให้การวัดมีความคลาดเคลื่อนและแม่นยำน้อยลง
    ```
...
| --- | -------------------------------------------------------------- | -------------------- | ------ |
| 1.1 | อธิบายหลักการทำงานของ LDR ได้ถูกต้อง                           |            4          | ฟังจากอาจารย์แล้วก็นำมาปรับใช้กับตัวเองเวลาต้องอธิบาย       |
| 1.2 | อธิบายการทำงานภายใน LDR (โฟตอน → อิเล็กตรอน → conduction band) |          4           |อธิบายได้บ้าง ตอนตอบคำถามอาจารย์ในชั้นเรียน        |
| 1.3 | เข้าใจว่าความสัมพันธ์ R vs Lux ไม่เป็นเส้นตรง                  |                4    |เข้าใจจากการอ่านใบความรู้ แต่ก็ยังมีส่วนที่ไม่มั่นใจ       |
| 1.4 | เข้าใจเหตุผลที่ต้องใช้ Voltage Divider                         |              5        | เข้าใจพราะว่า  Voltage Divider  สามารถแปลงเป็นแรงดันให้ ADC อ่านค่าได้     |

### **ส่วนที่ 10.2 — การต่อวงจรและอ่านค่า ADC (20 คะแนน)**

| ข้อ | รายการ                              | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | ----------------------------------- | -------------------- | ------ |
| 2.1 | ต่อวงจร Voltage Divider ได้ถูกต้อง  |           4           | หักออกไป 1 คะแนน เพราะดูแนวทางจาเพื่อนแล้วต่อด้วยตัวเอง        |
| 2.2 | เลือกค่า R คงที่เหมาะสม (เช่น 10kΩ) |            5          | อาจารย์เลือกมาให้อยู่แล้ว        |
| 2.3 | เขียนโปรแกรมอ่านค่า ADC ได้ถูกต้อง  |             4         | มีปรับโค้ดและรัน error ไปบ้างเลยหักไป 1 คะแนน        |
| 2.4 | ใช้ ESP‑IDF ADC + Calibration ได้   |         4             | พอใช้ได้เพราะอาจารย์สอนไปบ้างแล้ว |

### **ส่วนที่ 10.3 — การเก็บข้อมูลจริง (20 คะแนน)**

| ข้อ | รายการ                             | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | ---------------------------------- | -------------------- | ------ |
| 3.1 | เก็บข้อมูล ADC อย่างน้อย 5–7 จุด   |         5             |  เก็บข้อมูลได้ครบตามที่อาจารย์กไหนด      |
| 3.2 | วัดค่า Lux จากมือถือได้ถูกต้อง     |           4.5           | ค่าในมือถืออาจจะมีความคลาดเคลื่อนไปบ้างเลยตัดไป0.5       |
| 3.3 | จดข้อมูลเป็นตารางครบถ้วน           |         5             |    จดครบถ้วนเลยให้ 5 เต็ม    |
| 3.4 | ควบคุมสภาพแสงให้คงที่ระหว่างการวัด |           4           | ตัดไป 1 คะแนนเพราะเราไม่สามารถมั่นใจได้100%ว่าจะควบคุมแสงระหว่างวัดได้อย่างแม่นยำ        |
...
| ข้อ | รายการ                                        | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | --------------------------------------------- | -------------------- | ------ |
| 4.1 | สร้างกราฟ Scatter ADC vs Lux ได้              |           5           | สร้างได้เพราะอาจารย์มีตัวอย่างให้ดู       |
| 4.2 | เลือก Trendline เหมาะสม (Linear / Polynomial) |          5            | เหมาะสมเพราะอาจารย์สอน       |
| 4.3 | อ่านสมการจากกราฟได้ถูกต้อง                    |            5          | อาจารย์สอนดูสอนอ่าน        |
| 4.4 | นำสมการไปใช้ในโค้ดได้                         |           3           |  อาจจะยังไม่ดีพอ      |
| 4.5 | วิเคราะห์ว่า Linear หรือ Polynomial เหมาะกว่า |               5       |  วิเคราะห์ได้เพราะลองทำเปรียบเทียบกันหลายแบบๆแล้วนั่งดู พิจารณา      |
### **ส่วนที่ 10.5 — การทดสอบผลลัพธ์และวิเคราะห์ (15 คะแนน)**

| ข้อ | รายการ                                   | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | ---------------------------------------- | -------------------- | ------ |
| 5.1 | ทดสอบจุดแสงใหม่ที่ไม่ได้ใช้ตอนเก็บข้อมูล |            5          |   ทดสอบตาม lab     |
| 5.2 | เปรียบเทียบ Lux มือถือ vs Lux จากโค้ด    |         5             | เปรียบเทียบด้วยการวัด ดูตัวเลข วิเคราะห์ก็เข้าใจ       |
| 5.3 | วิเคราะห์ความคลาดเคลื่อนและสาเหตุได้     |           4           | วิเคราะห์ได้แต่หัก1 เพราะอาจจะยังไม่มั่นใจว่าจะถูกหรือไม่       |

###  **คะแนนรวมทั้งหมด:** ` 89.5 / 100`
...
1. จุดไหนในใบงานที่คิดว่ายากที่สุด?
    ...
    ```การปรับแสงให้อยู่ในระดับที่เหมาะสมตลอดระยะเวลาที่ทำ lab    ```
    
3. ถ้าทำใบงานนี้อีกครั้ง จะปรับปรุงอะไร?
 
   ```จะลองทำทุกอย่างด้วยตัวเองเตั้งแต่เริ่มต่อวงจร เขียนโค้ด พยายามด้วยตัวเองทุกวิถีทางให้มากขึ้น จะเก็บข้อมูลให้ละเอียดและหลายจุดมากขึ้น เพื่อที่จะได้การคำนวณที่แม่นยำกว่านี้   ```
    
5. ส่วนไหนที่คิดว่าตนเองเข้าใจดีมาก?
   
   ```การอ่านค่า ADC จาก LDR การต่อวงจร การสร้างกราฟใน excel และการแปลงเป็นค่า Lux ผ่านโปรแกรม ESP32   ```
